  ...
  for (U32 h = 0; h < m_H; ++h)
  {
    for (U32 j = 0; j < m_K + m_S; ++j)
    {
      if (IsBitSet(Mseq[j],h))
      {
        ...
        m_Degree[h + m_S]++; //统计加上第S到S+H-1行中对应的G_Half每行的度数
      }
    }
  }
  delete[] Mseq;
  ...
  for (U32 i = 0; i < m_N; ++i)
  {
    ...
    */
    ...
    for (U32 j = 1; j < jend; ++j)
    {
      ...
      m_A[i + m_S + m_H][b] = 1;
      m_Degree[i + m_S + m_H]++; //统计G_LT中每行的度数
    }   
  }
}
...
queue<CData> CDecoder::GetIntermediateSymbols(queue<CData> &encoded)
{
  ...
  queue<CData> recovedSym = SolveRevisedGauss(M, m_L, A, C, D);
